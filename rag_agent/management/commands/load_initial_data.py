...
from rag_agent.agents import ResearchAssistantAgent

class Command(BaseCommand):
    help = 'Load initial knowledge base into vector database'
    ...
    def handle(self, *args, **options):
        ...
        sample_file = os.path.join(kb_path, 'sample_papers.txt')
        if not os.path.exists(sample_file):
            with open(sample_file, 'w', encoding='utf-8') as f:
                f.write(self.get_sample_content())
            self.stdout.write(self.style.SUCCESS('Created sample papers'))
        
        agent = ResearchAssistantAgent(settings.VECTOR_DB_PATH)
        count = agent.load_knowledge_base(kb_path)
        
        self.stdout.write(
            self.style.SUCCESS(f'Successfully loaded {count} document chunks')
        )
